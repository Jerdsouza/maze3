...
void Maze::handleerror(ErrorNum error)
{
	switch (error)
	{
	case ErrorNum::MazeGrid:
		cout << "Please provide a valid  Maze Grid" << endl;
		break;
	case ErrorNum::NoStartCell:
		cout << "Did not find a start cell in the Maze grid" << endl;
		break;
	case ErrorNum::NumOfRowsIsZero:
		cout << "Please enter a valid number of rows greater than zero" << endl;
		break;
	case ErrorNum::NumOfColsIsZero:
		cout << "Please enter a valid number of columns greater than zero" << endl;
		break;
...
bool Maze::LoadMaze(int* gridArray, int numOfRows, int numOfCols)
{
	bool result = true;
	if (numOfRows == 0)
	{
